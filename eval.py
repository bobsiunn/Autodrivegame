...
#display 시각화를 위해 boxing, text 등 각종 처리를 하는 함수
...
#이미지의 class score, masked image의 score 등을 시각화해주는 함수
...
class Detections:
    ...
    #bbox와 mask 데이터 클래스 내 선언
    def __init__(self):
        self.bbox_data = []
        self.mask_data = []

    #bbox의 크기를 10의 배수로 저장하고, 라벨&score와 함께 저장하는 함수
    ...
    #mask image 데이터를 uint8 형식으로 encoding하고, 라벨&score와 함께 저장하는 함수
    ...
    #bbox와 mask data를 json 형태로 저장하는 함수
    def dump(self):
        dump_arguments = [
...
class APDataObject:
    ...
    def get_ap(self) -> float:
        ...
        #모든 정렬된 data score에 대해
        for datum in self.data_points:
            # datum[1] is whether the detection a true or false positive
            #정답인 scroe이면 true에 1을 더하고 아니면 false에 1 더함
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    #image가 입력되었을 때 evalimage 함수 호출
    ...
    #images가 입력되었을 때 evalimages 함수 호출
    elif args.images is not None:
        inp, out = args.images.split(':')
        evalimages(net, inp, out)
        return

    #video가 입력되었을 때 evalvideo 함수 호출
    ...
    #display를 arg로 주었을 때(연산 과정 시각화)
    if not args.display and not args.benchmark:
        # For each class and iou, stores tuples (score, isPositive)
        # Index ap_data[type][iouIdx][classIdx]

        #각 class에 대해 APDataObject를 호출하여 box와 mask key에 저장
        ap_data = {
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            #display 옵션이 주어진 경우 이미지 display 세팅
            #prep_display 함수 호출
            if args.display:
                img_numpy = prep_display(preds, img, h, w)
            
            #benchmark 옵션이 주어진 경우 score, class 등 연산 결과 수치 시각화 세팅
            #prep_benchmark 함수 호출
            elif args.benchmark:
                prep_benchmark(preds, h, w)

            #metrics 옵션인 경우, 이미지의 AP list 생성
            #prep_metrics 함수 호출
            else:
        ...
        #display 옵션인 경우 detection 결과를 dump하고 연산된 AP map 반환
        if not args.display and not args.benchmark:
            print()
            if args.output_coco_json:
        ...
        #benmark 옵션의 경우 FPS 및 실행 소요 시간 출력        
        elif args.benchmark:
            print()
            print()
...
if __name__ == '__main__':
    #parser argument 입력
    parse_args()
    #argument로 입력된 바가 있을 때, 입력값으로 config값 설정
    if args.config is not None:
        set_cfg(args.config)
    #argument train model이 interrupt 되었을 때, weights 디렉토리에 모델 저장
    if args.trained_model == 'interrupt':
        args.trained_model = SavePath.get_interrupt('weights/')
    #argument train model이 최신 버전일 때, name과 함께 weights 디렉토리에 모델 저장
    elif args.trained_model == 'latest':
        args.trained_model = SavePath.get_latest('weights/', cfg.name)

    #argument가 입력되지 않았을 때, train_model 이름으로 모델 경로를 저장하고, default 값 저장
    ...
    #detection task만이 목적인 경우, mask branch(semantic) 연산 pass
    if args.detect:
        cfg.eval_mask_branch = False
    #dataset이 입력되지 않은 경우, default값으로 설정
    if args.dataset is not None:
        set_dataset(args.dataset)

    #gradient 연산 시 메모리 용량 절약
    with torch.no_grad():
        #현재 경로에 results 폴더가 없으면 생성
        ...
        #resume이 arg로 주어질 시 data에 대해 map 연산
        ...
        #image, video, images가 입력값이 없으면 coco 데이터셋에서 가져옴
        ...
        #yolact 클래스 가져오기
        net = Yolact()
        #yolact 클래스에 정의된 weights 값 가져오기
